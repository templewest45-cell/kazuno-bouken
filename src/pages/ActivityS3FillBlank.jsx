...
function makeProblem(maxNumber) {
  const length = Math.min(5, maxNumber)
  const start = Math.floor(Math.random() * (maxNumber - length + 1)) + 1
  const values = Array.from({ length }, (_, index) => start + index)
  const blank = Math.floor(Math.random() * length)
  const answer = values[blank]
  values[blank] = null
  return { values, answer, choices: makeChoices(answer, 1, maxNumber) }
}

export default function ActivityS3FillBlank({ maxNumber = 10, activityId = 'S3_fill_blank_10', logActivity = 'fill_blank_10' }) {
  ...
  const choose = (number) => {
    ...
    LogStore.addLog({ stage: 'S3', activity: logActivity, answer: number, max: maxNumber, correct: true })
    speak(`せいかい！ ${number}`)
    setPhase('correct')
  }
  const next = () => {
    if (round + 1 >= maxQ) setPhase('complete')
    else { setRound((value) => value + 1); setProblem(makeProblem(maxNumber)); setPhase('play') }
  }
  return <S3Layout title={`${maxNumber}までの あなあき すうれつ`} subtitle="うみの つづきから、そらへ すすもう" onReset={reset}
