...
const STAGE_GUIDE = {
  ...
  3: {
    aim: '5までの数で系列の見方を確かめてから、10までに広げ、続き、逆順、前後の数を考える。',
    curriculum: '2段階「A 数と計算」。5までで活動のルールを理解し、10までの数の系列、順序や位置へ一般化する。',
    worksheetNote: '5まで・10までの穴あき数列と前後の数に対応。続き唱え、逆唱、複数穴の数列はプリントでは扱いません。',
  },
  4: {
    aim: '5までの具体物と数字記号の対応を固めてから、10までの数量と数字記号を結び付ける。',
    curriculum: '2段階「A 数と計算」。5までで数える・覚える・数字を選ぶ負荷を抑え、10までの数の数え方や表し方へ広げる。',
    worksheetNote: '5まで・10までの数字と数量の対応に対応。音読、カード並べ替えはプリントでは扱いません。',
  },
  5: {
  ...
  6: {
    aim: '5までの範囲で大小、順序、位置、間の数を確かめてから、10までの数へ広げる。',
    curriculum: '2段階「A 数と計算」。5までで比較や数直線の考え方をつかみ、10までの数の大小比較、系列、順序や位置へ一般化する。',
    worksheetNote: '5まで・10までの大小、数の道、間の数に対応。条件に合う数字探しはプリントでは扱いません。',
  },
}
...
export default function TeacherGuide() {
  return <main className="teacher-guide">
    ...
    <section className="teacher-guide-section">
      <h2>学習指導要領との関連</h2>
      <p>面の順番は、具体物への対応から始め、5までで活動のルールや考え方を確かめ、10までの数の系列、表し方、構成、大小や位置へ進む流れです。10までを扱う活動では、必要に応じて5まで版を先に使い、操作や問いの意味を理解してから10まで版へ進めます。</p>
      <div className="teacher-guide-table-wrap">
